#include<stdio.h>
#include<stdlib.h>
struct emp_struct{
    char *name;
    int id_no;
    char *dept_name;
    int age;
    int salary;
    char grade;
    struct student *next;
};
struct emp_struct *head;
void display()
{
    struct emp_struct *temp;
    temp=malloc(sizeof(struct emp_struct));
    temp=head;
    while(temp!=NULL)
    {
        //printf("    %d  ",temp->data);
        printf("ID NO:%d\n",temp->id_no);
        printf("NAME:%s\n",temp->name);
        printf("DEPT NAME:%s\n",temp->dept_name);
        printf("salary :%d\n",temp->salary);
        printf("age  :%d\n",temp->age);
        printf("GRADE:%c\n",temp->grade);
        temp=temp->next;
    }
}
void insert_beg()
{
    struct emp_struct *new_node;
    struct emp_struct *temp;
    temp=malloc(sizeof(struct emp_struct));
    new_node=malloc(sizeof(struct emp_struct));
    //printf("enter the node to be inserted\n");
    //scanf("%d",&new_node->data);
    printf("enter the id_no\n");
    scanf("%d",&new_node->id_no);
    new_node->name=(char*)malloc(20* sizeof(char));
    printf("enter the name\n");
    scanf("%s",new_node->name);
    new_node->dept_name=(char*)malloc(20* sizeof(char));
    printf("enter the dept_name\n");
    scanf("%s",new_node->dept_name);
    printf("enter the age marks\n");
    //scanf("%d",&new_node->marks->age);
    scanf("%d",&new_node->age);
    printf("enter the salary\n"); 
    scanf("%d",&new_node->salary);
    if(new_node->salary>=60000)
    {
        new_node->grade='A';
    }
    else if(new_node->salary>=50000 && new_node->salary<60000)
    {   new_node->grade='B';
    }
    else if(new_node->salary>=20000 && new_node->salary<50000)
    {    new_node->grade='C';
    }
    else
    {
        new_node->grade='F';
    }
    printf("e%c\n",new_node->grade);
    if(head==NULL)
    {
    head=new_node;
    new_node->next=NULL;
    //head=temp;
    }
    else
    {
        temp=head;
        head=new_node;
        new_node->next=temp;
        //temp=new_node;

    }
}
void insert_at_end()
{
    struct emp_struct *new_node;
    struct emp_struct *temp;
    temp=malloc(sizeof(struct emp_struct));
    new_node=malloc(sizeof(struct emp_struct));
    //printf("enter the node to be inserted\n");
    //scanf("%d",&new_node->data);
    printf("enter the id_no\n");
    scanf("%d",&new_node->id_no);
    new_node->name=(char*)malloc(20* sizeof(char));
    printf("enter the name\n");
    scanf("%s",new_node->name);
    new_node->dept_name=(char*)malloc(20* sizeof(char));
    printf("enter the dept_name\n");
    scanf("%s",new_node->dept_name);
    printf("enter the age marks\n");
    //scanf("%d",&new_node->marks->age);
    scanf("%d",&new_node->age);
    printf("enter the salary\n"); 
    scanf("%d",&new_node->salary);
    if(new_node->salary>=60000)
    {
        new_node->grade='A';
    }
    else if(new_node->salary>=50000 && new_node->salary<60000)
    {   new_node->grade='B';
    }
    else if(new_node->salary>=20000 && new_node->salary<50000)
    {    new_node->grade='C';
    }
    else
    {
        new_node->grade='F';
    }
    printf("e%c\n",new_node->grade);
    temp=head;
    if(head==NULL)
    {
        head=new_node;
        new_node->next=NULL;
    }
    else
    {
        while(temp->next!=NULL)
        {
            temp=temp->next;
        }
        temp->next=new_node;
        new_node->next=NULL;
    }
}

void insert_at_pos()
{
    int pos,i=1,count=1;
    struct emp_struct *temp;
    struct emp_struct *new_node;
    /*struct emp_struct *next;
    temp=malloc(sizeof(struct emp_struct));
    new_node=malloc(sizeof(struct emp_struct));
    next=malloc(sizeof(struct emp_struct));*/
    temp=malloc(sizeof(struct emp_struct));
    new_node=malloc(sizeof(struct emp_struct));
    printf("enter the position at which data is to be inserted\n");
    scanf("%d",&pos);
    printf("enter the id_no\n");
    scanf("%d",&new_node->id_no);
    new_node->name=(char*)malloc(20* sizeof(char));
    printf("enter the name\n");
    scanf("%s",new_node->name);
    new_node->dept_name=(char*)malloc(20* sizeof(char));
    printf("enter the dept_name\n");
    scanf("%s",new_node->dept_name);
    printf("enter the age marks\n");
    //scanf("%d",&new_node->marks->age);
    scanf("%d",&new_node->age);
    printf("enter the salary\n"); 
    scanf("%d",&new_node->salary);
   if(new_node->salary>=60000)
    {
        new_node->grade='A';
    }
    else if(new_node->salary>=50000 && new_node->salary<60000)
    {   new_node->grade='B';
    }
    else if(new_node->salary>=20000 && new_node->salary<50000)
    {    new_node->grade='C';
    }
    else
    {
        new_node->grade='F';
    }
    printf("e%c\n",new_node->grade);
    temp=head;
    while(temp!=NULL)
    {
        temp=temp->next;
        count++;
    }
    if(pos>count)
    {
        printf("these many number of nodes does not exit please enter within %d nodes",count);
    }
    else
    {
        temp=head;
        printf("in else\n");
        while(i<pos)
        {
            printf("in while\n");
            temp=temp->next;
            i++;
        }
        new_node->next=temp->next;
        temp->next=new_node;

    }
}

void delete_at_pos()
{
    int pos,i=1,count=0;
    struct emp_struct *temp,*next_node;
    temp=malloc(sizeof(struct emp_struct));
    next_node=malloc(sizeof(struct emp_struct));
    printf("enter the position at which data is to be deleted\n");
    scanf("%d",&pos);
    temp=head;
    while(temp->next!=NULL)
    {
        temp=temp->next;
        count++;
    }
    if(pos>count)
    {
        printf("the given position does not exit (limit %d)\n",count);
    }
    else
    {
        temp=head;
        while(i<pos)
        {
            temp=temp->next;
            i++;
        }

        next_node=temp->next;
        temp->next=next_node->next;
        free(next_node);

    }
}
void del_at_end()
{
    struct emp_struct *temp;
    struct emp_struct *prev_node;
    temp=malloc(sizeof(struct emp_struct));
    prev_node=malloc(sizeof(struct emp_struct));
    temp=head;
    while(temp->next!=NULL)
    {
        prev_node=temp;
        temp=temp->next;
    }
    prev_node->next=NULL;
    free(temp);
}
void del_at_front()
{
    struct emp_struct *temp;
    temp=malloc(sizeof(struct emp_struct));
    temp=head;
    if(temp==NULL)
    {
        printf("the list is empty\n");
    }
    else
    {
        head=head->next;
        free(temp);
    }
}
void search()
{
    struct emp_struct *temp;
    int search,i=1,flag=0;
    printf("enter the value to be searched\n");
    scanf("%d",&search);
    temp=head;
    if(head->id_no==search)
    {
        printf("found");
    }
    else
    {
    while(temp!=NULL)
    {
    if(temp->id_no==search)
    {
        printf("data found at %d location",i);
        printf("ID NO:%d\n",temp->id_no);
        printf("NAME:%s\n",temp->name);
        printf("DEPT NAME:%s\n",temp->dept_name);
        printf("salary :%d\n",temp->salary);
        printf("age  :%d\n",temp->age);
        printf("GRADE:%c\n",temp->grade);
        flag=1;
        
    }
    temp=temp->next;
    i++;
    }
    }
    if(flag==0)
    {
        printf("data not found\n");
    }

}
int main()
{
    int choice,count=0;
    while(choice!=0)
    {
        printf("\n1.insert at first \n2.insert end\n3.insert at a position \n4.display employee records\n5.delete_a_record_at_pos\n6.delete at end\n7.delete at front\n8.search\n");
        scanf("%d",&choice);
        switch(choice)
        {
        case(1):
        
        insert_beg();
        break;
        case(2):
        insert_at_end();
        break;
        case(3):
        insert_at_pos();
        break;
        case(4):
        display();
        break;
        case(5):
        delete_at_pos();
        case(6):
        del_at_end();
        break;
        case(7):
        del_at_front();
        break;
        case(8):
        search();
        break;
        default:
        exit(0);
    }
    }
    return 0;
}